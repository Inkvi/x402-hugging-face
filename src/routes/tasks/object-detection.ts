...
const DEFAULT_MODEL = DEFAULT_MODELS["object-detection"];

export function createObjectDetectionRoute(
  ...
  router.post("/:org?/:model?", x402Middleware, async (c) => {
    ...
    try {
      ...
      const result = await client.objectDetection(model, imageData);
