...
const DEFAULT_MODEL = DEFAULT_MODELS["text-classification"];

export function createTextClassificationRoute(
  x402Middleware: MiddlewareHandler<HonoEnv>
): Hono<HonoEnv> {
  const router = new Hono<HonoEnv>();

  // POST /v1/text-classification (uses default model)
  // POST /v1/text-classification/org/model-name (uses specified model)
  router.post("/:org?/:model?", x402Middleware, async (c) => {
    ...
    try {
      ...
      const result = await client.textClassification(model, body.inputs);
