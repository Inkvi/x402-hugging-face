...
const DEFAULT_MODEL = DEFAULT_MODELS["audio-classification"];

export function createAudioClassificationRoute(
  ...
  router.post("/:org?/:model?", x402Middleware, async (c) => {
    ...
    try {
      ...
      const result = await client.audioClassification(model, audioData);
